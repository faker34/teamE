	...
	$('#sdSelect').change(function(){
		...
		sdLayer = new ol.layer.Tile({
					source : new ol.source.TileWMS({
						url : 'http://wisejia.iptime.org:8080/geoserver/faker/wms?service=WMS', // 1. 레이어 URL
			             params : {
			               'VERSION' : '1.1.0', // 2. 버전
...
$('#bjdSelect').change(function(){
				...
				new ol.layer.Tile(
						{
							source : new ol.source.OSM(
									{
										url : 'http://api.vworld.kr/req/wmts/1.0.0/2C80741E-1FA4-3B2C-844A-4B2EA766D0F9/Base/{z}/{y}/{x}.png' // vworld의 지도를 가져온다.
									})
						}) ],
				view : new ol.View({
	...
	map.on('singleclick', function(evt) {
	    ...
	    element.innerHTML = `<a id="popup-closer" class="ol-popup-closer"></a> <div><p>${sd_y}</p><code>${sgg_x}</code></div>`;
